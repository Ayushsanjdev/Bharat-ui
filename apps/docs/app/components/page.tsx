      ...
      <tbody>
        {rows.map((p) => (
          <tr key={p.name} style={{ borderBottom: "0.5px solid var(--border)" }}>
            ...
            <td style={{ padding: "10px 12px" }}><code style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-dim)" }}>{p.default}</code></td>
...
function SectionHeader({ name }: { name: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
      <code style={{ fontSize: 16, fontWeight: 700, color: "var(--text)", fontFamily: "var(--font-mono)" }}>{name}</code>
...
function DemoCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: "var(--surface)", border: "0.5px solid var(--border)", borderRadius: 10, padding: 20 }}>
      <div style={{ fontSize: 11, color: "var(--text-dim)", fontFamily: "var(--font-mono)", marginBottom: 14 }}>{label}</div>
      {children}
    </div>
  );
}
...
export default function ComponentsPage() {
  ...
  return (
    <div style={{ fontFamily: "var(--font-sans)", background: "var(--bg)", minHeight: "100vh" }}>
      <Nav />
      <div style={{ maxWidth: 960, margin: "0 auto", padding: isMobile ? "40px 20px 80px" : "64px 32px 120px" }}>
